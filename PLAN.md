...
> 版本: 1.2 | 日期: 2026-07-20 | 状态: 本地维护模式 + 题库拉取/同步上线（已部署腾讯云 PM2，端口 3003）
...
| 6 | **拉取** | 本地维护页面从服务器全量拉取到本地（按 ID 覆盖，含 tags/技巧） |
| 7 | **同步** | 本地全量上传到服务器（按 ID 覆盖，含 tags/技巧） |

> 拉取/同步仅本地环境（`hostname === localhost/127.0.0.1/::1`）显示，
> 服务器/部署环境隐藏。地址与编辑密码通过 UI 输入并记忆于 localStorage。
...
- **本地旁路**：`hostname=localhost` 时 `apiFetch` 自动注入 `X-Local-Dev: 1` 头，
  服务端 `verifyEditMode` 见此头 + Host 判定即放行，本地维护页面免密进入
...
### 10.5 阶段二 Step 4 — ✅ 已完成（2026-07-20）本地维护模式 + 题库拉取/同步

Step 4 产出：本机访问 `/puzzles` 自动进入维护模式（免密），可从服务器拉取题库到本地、
或把本地题库同步到服务器；部署到服务器后这些按钮自动隐藏，回退为密码维护模式。

**新建文件：**
| 文件 | 说明 |
|------|------|
| `src/lib/env.ts` | `isLocalEnv()` 客户端判定 / `isLocalRequest()` 服务端 Host 判定 / `LOCAL_DEV_HEADER` 常量 |
| `src/components/PuzzleManager/SyncBar.tsx` | 同步工具栏（服务器地址+密码输入、拉取/同步按钮） |
| `src/app/api/puzzles/all/route.ts` | `GET` 全量导出题库（含 tags + techniqueNames） |
| `src/app/api/puzzles/upsert/route.ts` | `POST` 批量 upsert 题目（事务，按 ID 主键） |
| `src/app/api/tags/upsert/route.ts` | `POST` 批量 upsert 标签 |

**修改文件：**
| 文件 | 变更 |
|------|------|
| `src/lib/utils.ts` | `verifyEditMode` 加本地旁路：`Host=localhost + X-Local-Dev:1` 直接放行 |
| `src/hooks/useEditMode.ts` | 本地环境 `toggleEditMode` 免密进入；`apiFetch` 自动注入本地头 |
| `src/app/puzzles/page.tsx` | 本地环境初始即维护模式；集成 SyncBar；服务器环境保留密码 Modal |
| `src/components/Layout/AppHeader.tsx` | `/puzzles` 路径下隐藏「选择题目/数独技巧/题库维护」三按钮 |
| `src/config/constants.ts` | 新增 `SYNC_SERVER_KEY` / `SYNC_PASSWORD_KEY` |

**同步流程（按 ID upsert）：**
- 拉取：服务器 `GET /api/puzzles/all` → 本地 `POST /api/puzzles/upsert`
- 同步：本地 `GET /api/puzzles/all` → 服务器 `POST /api/puzzles/upsert`

**部署踩坑：**
1. 服务器 `/opt/mysudoku` 工作区有大量未跟踪文件（历史 scp 部署残留），通过
   `git reset --hard origin/master` + `git clean -fd` 清理孤儿文件（`ChainBuilder/`、
   `ChainCanvas.tsx`、`useChainDrawing.ts` 等，这些文件不在仓库但 TypeScript 编译时会扫到导致 build 失败）
2. 服务器 `edit_password` 实际为 `mysudoku`（用户早期数据库保留值），而非
   `db.ts` 中的新默认 `Kili+2016`（`INSERT OR IGNORE` 不会覆盖已有值）
3. pm2 实际监听 3003 端口（非 PLAN.md 文档里写的 3000）
4. nginx 没有为 mysudoku 配置反代，目前通过公网 IP:3003 直连

**验证：** tsc 0 错误 / lint 无新增错误（基线 3 errors / 113 warnings）/ 生产 build 通过 /
本机双向同步测试通过（本机 2 题 → 服务器 0 题，同步后两端均 2 题，ID 一致，techniqueNames 正确）。
